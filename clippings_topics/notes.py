...
one thing. This app contributes what the clipped-article corpus knows about a
topic; `podcast-digest` and `security-digest` do the same for theirs; and the
person whose vault it is writes their own thinking at the top. All want the
*same* file, because splitting them would put two ``anthropic.md`` in the vault, leave links
...
We never hold a copy of a topic note at all -- the vault is the only source -- nothing syncs back
...
copy and the first implementation of this contract; `security-digest`
(`src/vault/notes.py`) is the second, and this is the third. Only :data:`OWNER`
and :data:`KEY_PREFIX` differ, plus the absence of podcast-digest's
legacy-adoption machinery: this app has never written a topic note in any other
format, so there is nothing here to adopt. Fix bugs in all three copies or none
-- a divergence here corrupts notes the other applications also write to.

This writer is unusual in one way worth knowing: it is the only one that
*discovers* what to write about by reading the vault (``10 raw/``) rather than
being told. See ``vault.list_prefix``.
"""
...
def merge_frontmatter(
    ...
    * **Prefixed** (``clip_count``) are ours. Replaced every run, and
    ...
    * **Unprefixed** (``type``, ``title``, ``tags``) describe the note as a
    ...
    for line in existing:
        ...
        # of another writer's `security_*` block. No sequence of this function
        # can produce that (it always appends the owner's keys last, and these
        # sat either side of another writer's), and 4 of the 21 had no region of
        # ours at all. The likely source is LiveSync's own line-level merge of
        # two revisions during concurrent writes, which duplicates identical
        ...
        # both. This heals the common case and never decides the ambiguous one.
        if line in kept:
            continue
        kept.append(line)
