...
**Ported from podcast-digest** (`podcast_agent/vault.py`), which is the canonical
implementation; `taster/backend/app/couchdb_client.py` was the first. The format
is not documented by the plugin — it was reverse-engineered from what a live
v0.25 client writes, and is in production in three projects now. Fix bugs in all
of them or none.
...
What this copy adds: :meth:`LiveSyncVault.list_prefix`. Every other writer only
touches notes whose path it already knows — it is told what to publish. This one
has to *discover* what to write about by reading ``10 raw/``, and nothing in the
original supports that.
...
class LiveSyncVault:
    ...
    async def list_prefix(self, prefix: str) -> list[Entry]:
        """Every live file under ``prefix``. The one capability the original lacks.
        ...
        **``include_docs=true`` is not an optimisation, it is the correctness
        ...
        ``_all_docs`` lists deleted notes exactly like present ones, and the flag
        is only visible in the document. Measured on this vault: 141 rows for 109
        real notes. Trusting the row list would mean publishing links to 32 notes
        that exist on no device.

        Filtered here rather than by the caller so there is one place to get it
        wrong.
        """
        ...
        log.info("vault.listed prefix=%s live=%d soft_deleted=%d", prefix, len(entries), skipped)
    ...
    async def project(self, path: str, markdown: str, *, mtime_ms: int, merge: bool = True) -> bool:
        ...
        Skipped means the note already says exactly this, or a human deleted it
        and that deletion is being respected.
        """
        if self._client is None:
            raise VaultUnavailable("vault.couchdb_url is not set")
    ...
    async def _put_chunk(self, chunk_id: str, markdown: str) -> None:
        ...
        whose children cannot be fetched is a file that renders empty.
        """
    ...
    async def soft_delete(self, doc_id: str) -> bool:
        """Delete a note the way Obsidian does. False if there was nothing to delete.
        ...
        clients — a real ``DELETE`` leaves them holding a file the server can no
        longer describe, and they put it straight back.

        So this is a read-modify-write of the exact shape a client produces,
        which is the whole reason it is safe to use on notes a person can see.
        """
        assert self._client is not None
        ...
        log.info("vault.soft_deleted path=%s", doc.get("path") or doc_id)
        return True
